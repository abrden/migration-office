...
TEST_CASE("ConfigurationFileReader") {
    ...
    REQUIRE(serialized_people.front().second == "1,38464269,,Agustina,Barbetta,tez blanca,ojos oscuros");
    REQUIRE(serialized_people.back().first == 2);
    REQUIRE(serialized_people.back().second == "0,1234567,PORTOÑOL123,Manuel,Porto,tez blanca,ojos verdes,barbudo");
}